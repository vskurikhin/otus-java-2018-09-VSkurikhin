...
public class StatisticCustomTagService implements DataOrigin, StatisticService
{
    ...
    public long saveStatisticFromRequestParams(HttpServletRequest request, DbService dbService)
    {
        ...
        StatisticController controller = (StatisticController) dbService.getController(StatisticEntity.class);

        try {
            return controller.insertProcedure(result);
        } catch (ExceptionThrowable exceptionThrowable) {
            LOGGER.error("ERROR insert statistic: {}", exceptionThrowable);
        }
